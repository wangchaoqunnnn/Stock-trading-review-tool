...
def quote(secid):
    params = {
        "fltt": 2,
        "invt": 2,
        "fields": "f2,f3,f6,f8,f10,f12,f14,f17,f18,f20,f21,f22,f23,f24,f25,f26,f62,f184,f100,f103,f116,f117",
        "secids": secid,
    }
...
def news():
    hits = []
    keywords = ["至纯科技", "半导体设备", "清洗设备", "晶圆", "国产替代", "中芯", "订单"]
...
def main():
    ...
    for i, r in enumerate(rows[-40:], start=len(rows) - 40):
        ...
        print(
            ...
            f"{r['turnover']:5.2f} {vr:5.2f} {ma5 if ma5 is None else round(ma5,2)} "
    ...
    for i, r in enumerate(recent):
        ...
        desc = f"{r['date']} 开{r['open']:.2f} 收{r['close']:.2f} 涨跌{r['pct']:.2f}% 换手{r['turnover']:.2f}%"
        if prev:
            vr = r["volume"] / prev["volume"] if prev["volume"] else 0
            desc += f" 量比昨{vr:.2f}"
        print(desc)
